...
//________________________________________________________________SmoothSurface
SmoothSurface::SmoothSurface (numeric::array X,
                  tuple frame,
...
//_______________________________________________________________~SmoothSurface
...
SmoothSurface::initialize (void)
{
	...
	for (int j=0; j < int(d1-1); j++) {
        for (int i=0; i < int(d0-1); i++) {
            ...
            indices[index++] = u;
            indices[index++] = u+1;
            indices[index++] = u+d0+1;
            indices[index++] = u+d0;
        }
    }

    float dx = (1.0f / (d0-1))*w;
	float dy = (1.0f / (d1-1))*h;    
	for (int j=0; j < int(d1); j++) {
        for (int i=0; i < int(d0); i++) {
            vertices[i + j*d0].x = x+i*dx-.5;
            vertices[i + j*d0].y = y+j*dy-.5;
            vertices[i + j*d0].z = 0;
        }
    }
...
SmoothSurface::update (void)
{
    ...
    for (unsigned int j=0; j<this->d1; j++) {
        ...
        for (unsigned int i=0; i<this->d0; i++) {
            ...
            colors[index].alpha  = d[3]*this->alpha;
...
void
SmoothSurface::python_export (void)
{

    using namespace boost::python;
    register_ptr_to_python< boost::shared_ptr<SmoothSurface> >();
    import_array();
    numeric::array::set_module_and_type ("numpy", "ndarray");  

    class_<SmoothSurface, bases< Array> > ("SmoothSurface",
