...
from glpython.terminal.terminal import Terminal
from glpython.terminal.iterminal import Terminal as ITerminal
...
def window (namespace={}, w=650, h=400, title='GLPython', fps=30,
    ...
    class Window (base):
        """ """
        def __init__ (self, w=650, h=400, title='OpenGL window', fps=30.0):
            """ """
        ...
        def resize (self, w, h):
            ...
            if self.terminal:
                self.terminal.reshape (self.width, self.height)
        ...
        def key_press (self, key):
            """ """
            if key == 'control-d':
                if not self.terminal or self.terminal.current_line() == '':
                    self.destroy()
    ...
    if shell == 'python':
        _window.terminal = Terminal (namespace, _window.paint, _window.idle)
    else:
        _window.terminal = ITerminal (namespace, _window.paint, _window.idle)
